...
This release increments the already-created `0.0.4` package to `0.0.5`. It retains the four helpers under `scripts/` and their configured `/root/Source/MQTT-command-executioner/scripts/` paths. The service now uses `/root/Source/MQTT-command-executioner/` as `WorkingDirectory`, `/root/Source/MQTT-command-executioner/mqtt-listener.py` as its script, and `/root/Source/MQTT-command-executioner/commands.txt` as its config.

The final archive contains 23 files. All version 0.0.4 paths remain. Sixteen files are byte-identical to 0.0.4; seven have intentional changes: README, VERSION, VERSIONING.md, commented_code_map.md, mqtt-listener.service, VERIFICATION.md, and manifest.sha256. The four helpers retain their content and executable permissions.
...
- The existing example test verifies all seven configuration settings/sections, all four original power payloads, the exact `/root/Source/MQTT-command-executioner/scripts` prefix, and the packaged helper files under `scripts/`.
...
- Listener source and all helper contents match versions 0.0.3 and 0.0.4 byte-for-byte. The status safeguards, parser fix, exact command lookup, shell execution, and startup behavior are unchanged.
- The exact service working directory and script/config paths match the requested destination. All other service directives match version 0.0.4.
...
The supplied service paths are updated. Replace its placeholder account and, if using a virtual environment, select its Python interpreter using README before installation. The listener account needs access to `/root/Source/MQTT-command-executioner` when using these paths.
